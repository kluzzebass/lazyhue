...
func NewColorTempSliderComponent(id, label string, value, min, max int, styles *ui2.Styles, zones *zone.Manager) *ColorTempSliderComponent {
...
func (c *ColorTempSliderComponent) renderColorTempBar(value, min, max, width int, styles *ui2.Styles) string {
	if max <= min {
		return ""
	}
